...
uint16_t file_buff[FILE_BUFF_SIZE];
FRESULT fresult; 
FIL file_obj; 
...
**********************************************************************/
FRESULT lcd_font16(uint16_t x,uint16_t y,uint16_t fc,uint16_t bc,char *s,char *font)
{
	uint8_t qh=0,wh=0;
	uint32_t offset; 
	uint32_t br; 
	...
	if ( !strcmp(font,"heiti16.zk") )
	{
		pEn = &file_16_ht_en;
		pHz = &file_16_ht_ch;
	}
	else if ( !strcmp(font,"song16.zk") )
	{
		pEn = &file_16_st_en;
		pHz = &file_16_st_ch;
	}
...
**********************************************************************/
FRESULT lcd_font24(uint16_t x,uint16_t y,uint16_t fc,uint16_t bc,char *s,char *font)
{
	uint8_t qh=0,wh=0;
	uint32_t offset; 
	uint32_t br; 
	...
	if ( !strcmp(font,"heiti24.zk") )
	{
		pEn = &file_24_ht_en;
		pHz = &file_24_ht_ch;
	}
	else if ( !strcmp(font,"song24.zk") )
	{
		pEn = &file_24_st_en;
		pHz = &file_24_st_ch;
	}
...
**********************************************************************/
FRESULT lcd_font20(uint16_t x,uint16_t y,uint16_t fc,uint16_t bc,char *s,const char *font)
{
	uint8_t qh=0,wh=0;
	uint32_t offset; 
	uint32_t br; 
...
**********************************************************************/
FRESULT lcd_font32(uint16_t x,uint16_t y,uint16_t fc,uint16_t bc,char *s,char *font)
{
	uint8_t qh=0,wh=0;
	uint32_t offset; 
	uint32_t br; 
	...
	if ( !strcmp(font,"heiti32.zk") )
	{
		pEn = &file_32_ht_en;
		pHz = &file_32_ht_ch;
	}
	else if ( !strcmp(font,"song32.zk") )
	{
		pEn = &file_32_st_en;
		pHz = &file_32_st_ch;
	}
...
/**********************************************************************
functionName:void font_load_init( void )
description:字体加载初始化
**********************************************************************/
void font_init( void )
{
