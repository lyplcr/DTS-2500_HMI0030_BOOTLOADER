...
** IDE:MDK4.5 
2015/6/24:
1、驱动程序改编自安富莱电子串口驱动，支持多串口驱动
2、没有把所有的串口驱动写全，因为没法测试，后续只需要修改UartHardInit、UartNVICConfig两个函数即可，需要增加新的串口UARTx_FIFO_EN定义为1即初始化该串口
...
#if UART6_FIFO_EN == 1		/* 串口1 TX = PC6   RX = PC7 或 */

	USART_DeInit(USART6);
	
	/* 第1步： 配置GPIO */
	#if 1	/* 串口1 TX = PC6   RX = PC7 */
		/* 打开 GPIO 时钟 */
