...
static void DownPointCalculateCycle( uint8_t chn );
...
{	
	...
	/* 获取判破相关数据 */
	DownPointCalculateCycle(SMPL_KY_NUM);
	DownPointCalculateCycle(SMPL_KZ_NUM);
}
...
/*------------------------------------------------------------
 * Function Name  : DownPointCalculateCycle
 * Description    : 计算下降点
 * Input          : None
 * Output         : None
 * Return         : None
...
static void WritePrmParameterInit( void )
{
	...
	g_WritePrm.len = PRM_MEM_SIZE_2CH;
}				
		...
		case LINK_CHECK_PRM:
			...
			if (g_WritePrm.len > PRM_BUFF_CHECK)
			{
				...
				if (PASSED == compStatus)
				{
					...
					g_linkMutual.process 	= (float)(PRM_MEM_SIZE_2CH - g_WritePrm.len) / PRM_MEM_SIZE_2CH * 100;
				}
				else
				{
